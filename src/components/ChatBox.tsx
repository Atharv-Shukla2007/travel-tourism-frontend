...
const ChatBox = ({ bookingId, senderRole }: ChatBoxProps) => {
            ...
            <div
              className={`max-w-[75%] px-4 py-2.5 text-sm ${
                msg.sender === senderRole
                  ...
                  : "bg-background text-foreground rounded-2xl rounded-tl-none card-shadow"
              }`}
            >
              {msg.message}
            </div>
      ...
      <div className="p-3 border-t border-border/50">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 rounded-xl bg-background text-sm text-foreground placeholder:text-muted-foreground focus:outline-none card-shadow"
          />
          <button
            onClick={handleSend}
