    ...
    <div className="rounded-2xl card-shadow hover:card-shadow-hover transition-all duration-300 bg-background p-5">
        ...
        <div className="relative">
          ...
          <div
            className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-background ${
              expert.status === "available" ? "bg-emerald" : "bg-muted-foreground"
            }`}
          />
        </div>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        {expert.languages.map((lang) => (
          <span key={lang} className="text-[10px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-md bg-secondary text-muted-foreground">
            {lang}
          </span>
        ))}
      </div>
      ...
      {user?.role === "user" && (
        <div className="mt-4 flex gap-2">
          <button
            onClick={handleBook}
            disabled={alreadyBooked || expert.status === "busy"}
