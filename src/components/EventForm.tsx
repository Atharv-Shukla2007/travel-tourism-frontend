...
import { TourEvent } from "@/data/events";
import { Plus } from "lucide-react";
import { toast } from "sonner";

interface EventFormProps {
  onSubmit: (event: TourEvent) => void;
}
...
const EventForm = ({ onSubmit }: EventFormProps) => {
  ...
  const [form, setForm] = useState({
    ...
    date: "",
    description: "",
    category: "upcoming" as TourEvent["category"],
  });
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl card-shadow bg-background p-5 space-y-3 animate-scale-in">
      ...
      <select
        value={form.category}
        onChange={(e) => setForm({ ...form, category: e.target.value as TourEvent["category"] })}
      ...
      <div className="flex gap-2">
        <button
          type="submit"
          className="flex-1 py-2.5 rounded-xl text-sm font-medium bg-primary text-primary-foreground hover:opacity-90 active:scale-95 transition-all"
        >
          Create
        </button>
        <button
          ...
          className="flex-1 py-2.5 rounded-xl text-sm font-medium bg-secondary text-foreground hover:bg-secondary/80 transition-colors"
        >
          Cancel
        </button>
      </div>
