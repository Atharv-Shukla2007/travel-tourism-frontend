        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <div>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter your name"
              className="mt-1.5 w-full px-4 py-3 rounded-xl bg-secondary text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring/20 transition-shadow" required />
          </div>
          <div>
            ...
            <div className="mt-1.5 flex p-1 bg-secondary rounded-xl">
              ...
              <button type="button" onClick={() => setRole("expert")}
                ...
                <BookOpen className="w-4 h-4" />Expert
              </button>
            </div>
          </div>
