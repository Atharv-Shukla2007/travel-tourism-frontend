    ...
    <div className="pt-16">
      <div className="max-w-7xl mx-auto px-4 py-12 space-y-20">
        <div>
          ...
          <p className="text-muted-foreground mt-2 max-w-lg leading-relaxed">Your gateway to monuments, events, experts, and heritage tools.</p>
        </div>
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredMonuments.map((m) => <MonumentCard key={m.id} monument={m} />)}
          </div>
          {filteredMonuments.length === 0 && <p className="text-sm text-muted-foreground text-center py-12">No monuments found. Try adjusting your filters.</p>}
        </section>
        <section>
          ...
          <div className="flex gap-2 mb-6">
            {["all", "ongoing", "upcoming", "past"].map((cat) => (
              <button key={cat} onClick={() => setEventCategory(cat)}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${eventCategory === cat ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"}`}>
        ...
        <section>
          ...
          <div className="max-w-xl"><Scanner /></div>
        </section>
        <section>
